...
class BaseController extends Controller {
	...
	public function __construct(){
        ...
        $topic_comments = $this->changyan_topic_comments();
        $this->assign("recentcomment",$topic_comments);
    ...
    //文章评论信息批量获取
    private function changyan_topic_count(){

        $row = $this->ArticleModel->field("article_id")->select();
        ...
        $request_args = array(
            "client_id"=>'cyt2w7YqF',
            "topic_id"=>'',
            "topic_source_id"=>implode(',',$data),
            "topic_url"=>'',
            "callback"=>''
        );

        $res = curlget($url,$request_args);
    ...
    private function changyan_topic_comments(){

        $topicidlist = $this->changyan_topic_count();

        foreach ($topicidlist['result'] as $k=>$v) {
            if($v['sum']>0){
                $row[] = $v['id'];
        ...
        foreach ($row as $k=>$v){
            ...
            foreach ($res['comments'] as $vv){
                $communication[] = $vv;
            }

        }

        foreach ($communication as $k=>$v){
            $communication_have[$k]['content'] = $v['content'];
            $communication_have[$k]['ctime'] = $v['create_time'];
            $communication_have[$k]['create_time'] = date('Y-m-d H:i:s',$v['create_time']/1000);
            $communication_have[$k]['author_img'] = $v['passport']['img_url'];
            $communication_have[$k]['author_name'] = $v['passport']['nickname'];
        }


        //排序
        foreach($communication_have as $v){
            $flag[] = $v['ctime'];
        }

        array_multisort($flag, SORT_DESC, $communication_have);

        return $communication_have;
